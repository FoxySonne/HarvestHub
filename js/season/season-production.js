...
import { formatHoursMinutes } from "./time-format.js";
...
function setText(id, value) {
  const element = document.getElementById(id);
  if (!element) return;
  element.textContent = typeof value === "string" ? value : numberFormat.format(value);
}
...
function getSelectedBuildingProduction() {
  return getByLevel(seasonDatabase.productionByBuildingLevel, num("productionBuildingLevel")) || { secondary: 0, primary: 0 };
}

function getFactoryProduction() {
  const ids = ["secondary_factory_1", "secondary_factory_2", "primary_factory_1", "primary_factory_2"];
  ...
  if (!hasAllRows) return fallback;

  const secondaryLevels = [getBuildingCurrentLevel("secondary_factory_1"), getBuildingCurrentLevel("secondary_factory_2")];
  const primaryLevels = [getBuildingCurrentLevel("primary_factory_1"), getBuildingCurrentLevel("primary_factory_2")];

  const secondary = secondaryLevels.some(level => level > 0)
    ? (Number(getBuildingProduction("secondary_factory_1").secondary) || 0) + (Number(getBuildingProduction("secondary_factory_2").secondary) || 0)
    : Number(fallback.secondary) || 0;
  const primary = primaryLevels.some(level => level > 0)
    ? (Number(getBuildingProduction("primary_factory_1").primary) || 0) + (Number(getBuildingProduction("primary_factory_2").primary) || 0)
...
function updateProduction() {
  ...
  const secondaryHours = needSecondary <= 0 ? 0 : production.secondaryPerHour > 0 ? needSecondary / production.secondaryPerHour : Number.POSITIVE_INFINITY;
  const primaryHours = needPrimary <= 0 ? 0 : production.primaryPerHour > 0 ? needPrimary / production.primaryPerHour : Number.POSITIVE_INFINITY;
  ...
  setText("productionNeedHours", Number.isFinite(rawNeedHours) ? formatHoursMinutes(rawNeedHours, { rounding: "ceil" }) : "—");
}
