...
function applyMainBuildingLimits({ notify = true } = {}) {
  ...
  try {
    ...
    if (notify) [...new Set(changedFields)].forEach(notifyFieldChanged);
  } finally {
...
function initSeasonBuildingLinks() {
...
const observer = new MutationObserver(() => initSeasonBuildingLinks());
function start() {
...
window.initSeasonBuildingLinks = initSeasonBuildingLinks;
